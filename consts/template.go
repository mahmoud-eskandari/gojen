...
type {{.SingularName}} struct{
	...
	ctx		*goje.ContextHandler {{structTag "-"}}
	parent  goje.Entity {{structTag "-"}}
}

type {{.SingularName}}List []{{.SingularName}}
var BeforeInsert{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}}) error
var BeforeUpdate{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}}) error
var BeforeDelete{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}}) error
var AfterInsert{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}})
var AfterUpdate{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}})
var AfterDelete{{.SingularName}} func(*goje.ContextHandler, *{{.SingularName}})

const {{.SingularName}}TableName = "{{.Name}}"
{{if .Primary}}const {{.SingularName}}PrimaryKey = "{{.Primary}}"{{end}}
var {{.SingularName}}Columns = []string{
	{{range $ir,$col := .Columns}}"{{$col.Name}}",{{end}}
}

/* TODO
var {{.SingularName}}DefaultEnabledRelations = []goje.Relation{
	{{range .LRelations}}
	//Load {{camel .Table.SingularName}} by {{.FromCol}}
	goje.Relation{
...
func (opt {{.SingularName}}) GetCtx() *goje.ContextHandler {
	return opt.ctx
}

//SetCtx set context handler for current object
func (opt {{.SingularName}}) SetCtx(ctx *goje.ContextHandler) {
	opt.ctx = ctx
}
...
func Delete{{.SingularName}}Raw(handler *goje.ContextHandler, Queries ...goje.QueryInterface) (rowsAffected int64, err error) {
	return handler.RawDelete("{{.Name}}", Queries)
}

// Insert multiple {{.SingularName}}
// This method dosen't support After,Before Triggers ...
func BulkInsert{{.SingularName}}(handler *goje.ContextHandler, entities []{{.SingularName}}) (rowsInserted int64, err error) {
...
func (opt *{{.SingularName}}) Delete(handler *goje.ContextHandler) error {
	if handler == nil {
		handler = opt.ctx
	}
	if handler == nil {
...
func (opts *{{.SingularName}}List) DeleteAll(handler *goje.ContextHandler) error {
...
func (opt *{{.SingularName}}) Save(handler *goje.ContextHandler) error {
	if handler == nil {
		handler = opt.ctx
	}
	if handler == nil {
...
func (opt *{{.SingularName}}) Insert(handler *goje.ContextHandler) error {
	...
	result, err := handler.DB.ExecContext(handler.Ctx, ` + "`" + `INSERT INTO {{.Name}}({{join (NonPrimaryCols .) ","}}) VALUES({{join (NonPrimaryColsArgs .) ","}})` + "`" + `{{range $ir,$col := (NonPrimaryCols .)}},opt.{{camel $col}}{{end}})
	if err != nil {
		return err
	}
	{{if ne .Primary ""}}
...
//Update an entry
func (opt *{{.SingularName}}) Update(handler *goje.ContextHandler) error {
	if handler == nil {
		handler = opt.ctx
	}
	...
	_, err := handler.DB.ExecContext(handler.Ctx, "UPDATE {{.Name}} SET {{range $ir,$col := (NonPrimaryCols .)}}{{if ne $ir 0}},{{end}}{{$col}}=?{{end}} WHERE {{.Primary}}=?"{{range $ir,$col := (NonPrimaryCols .)}},opt.{{camel $col}}{{end}},opt.{{camel .Primary}})
	if err != nil {
		return err
	}
...
//Get{{.SingularName}}By{{camel .Primary}} Get {{.SingularName}} By {{camel .Primary}}
func Get{{.SingularName}}By{{camel .Primary}}(handler *goje.ContextHandler, {{camel .Primary}} interface{}) (*{{.SingularName}}, error) {
	if handler == nil {
		return nil, goje.ErrHandlerIsNil
	}

	row := handler.DB.QueryRowContext(handler.Ctx, "SELECT {{range $ir,$col := .Columns}}{{if ne $ir 0}},{{end}}{{$col.Name}}{{end}} FROM {{.Name}} WHERE {{.Primary}}=?",{{camel .Primary}})
...
func Get{{$.SingularName}}By{{range $ic,$col := $cols}}{{if ne $ic 0}}And{{end}}{{camel $col.Name}}{{end}}(handler *goje.ContextHandler{{range $cols}}, {{camel .Name}} interface{}{{end}}) (*{{$.SingularName}}, error) {
	if handler == nil {
		return nil, goje.ErrHandlerIsNil
	}
...
func Get{{camel .Name}}(handler *goje.ContextHandler, Queries ...goje.QueryInterface) (*[]{{.SingularName}}, error) {
	if handler == nil {
		return nil, goje.ErrHandlerIsNil
	}
...
func Count{{camel .Name}}(handler *goje.ContextHandler, Queries ...goje.QueryInterface) (int, error) {
	if handler == nil {
