...
func Get{{.SingularName}}By{{camel .Primary}}(handler *goje.Context, {{camel .Primary}} interface{}) (*{{.SingularName}}, error) {
	if handler == nil {
...
func Get{{$.SingularName}}By{{range $ic,$col := $cols}}{{if ne $ic 0}}And{{end}}{{camel $col.Name}}{{end}}(handler *goje.Context{{range $cols}}, {{camel .Name}} interface{}{{end}}) (*{{$.SingularName}}, error) {
	if handler == nil {
		return nil, goje.ErrHandlerIsNil
	}
